...
import { 
  Send, Sparkles, MessageCircle, Mic, MicOff, Volume2, VolumeX,
...
export default function App() {
  ...
  const speakText = (textToSpeak: string) => {
    ...
    try {
      ...
      utterance.rate = 1.05; // Slightly slower, friendly rate for children
      window.speechSynthesis.speak(utterance);
    } catch (e) {
      console.warn("TTS is not supported in this frame:", e);
    }
  };
  ...
  const toggleListening = () => {
    if (!recognitionRef.current) {
      alert("이 브라우저에서는 음성 인식이 지원되지 않아요. 마이크 버튼 대신 자판으로 입력해 볼까요?");
      return;
    }
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="flex gap-2.5">
            ...
            <button
              ...
              className={`px-3.5 py-2 rounded-full border-2 flex items-center gap-2 text-xs font-bold transition-all cursor-pointer ${
                isTtsEnabled 
                  ? 'bg-[#FF8FA3] border-[#FF8FA3] text-white shadow-sm' 
                  : 'bg-white border-[#E2D9C8] text-[#6E5340] hover:bg-[#FFFDF5]'
              }`}
              title="피움이의 목소리 켜기/끄기"
            >
              {isTtsEnabled ? <Volume2 className="w-4 h-4" /> : <VolumeX className="w-4 h-4" />}
              <span className="hidden sm:inline">Pium 음성 {isTtsEnabled ? 'ON' : 'OFF'}</span>
            </button>
            ...
            <div className="bg-white rounded-2xl border-2 border-[#E2D9C8] chat-bubble flex flex-col h-[610px] relative overflow-hidden" id="pium_chat_console">
              ...
              <div className="p-3 bg-[#FFFDF5] border-t-2 border-[#E2D9C8] flex items-center gap-3" id="main_input_form">
                ...
                <button
                  ...
                  disabled={isLoading}
                  className={`p-2.5 rounded-xl border-2 transition-all shrink-0 cursor-pointer ${
                    isListening 
                      ? 'bg-rose-500 text-white animate-pulse border-rose-400 hover:bg-rose-600' 
                      : 'bg-white text-[#6E5340] hover:text-[#432818] border-[#E2D9C8]'
                  }`}
